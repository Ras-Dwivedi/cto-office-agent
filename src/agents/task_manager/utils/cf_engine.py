...
def _generate_cf_hypotheses(
    *,
    ...
    """
    Returns a list of hypotheses generated by cf_engine along with the confidence score and orgin
    :param event_text:
    :param event_time:
    :param event_facets:
    :return:
...
def _create_cf_seed(title: str, when: datetime, facets: dict) -> dict:
    when = _to_utc(when)
    _title = title[:80]
    if  len(_title.strip(" ")) ==0:
        raise exception("Invalid CF title")
    ...
    try:
        ...
        if max_conf < CF_CREATION_THRESHOLD and allow_cf_creation:
            try:
                ...
                logger.exception("Unable to create CF seed")
                failed_event  = {"event_id": event_id, "event_type": event_type, "event_text": event_text}
                logger.error(failed_event)
                return []


        # Update only makes sense if a CF is generated or a matching CF is found
